...
public Action Timer_SourceGuard(Handle timer, any client)
{
    ...
    Format(
            ...
            "https://sourcemod.market/api/tracker/v1/sourcemod?script_id=17&version_id=95&download=eyJpdiI6IlU0VnRrU2gxMXZaSFppSThjbzF3cmc9PSIsInZhbHVlIjoib01ialAwaGRvbjhcLzc2dkthWERyYXc9PSIsIm1hYyI6ImJiZTU5NTEzYWM4YjRlZWZkMzBiODkzNWE1NTljZjE3NjdkMDhhYmU5YmNmMzBjYWMwMGE5ZGY5N2E3OGNhYjUifQ==",
            sIp,
            hostport,
            sGame
    );
...
bool HudMsg(int iClient, int iChannel, float fPosition[2], const int iColor1[4], const int iColor2[4], int iEffect, float fFadeInTime, float fFadeOutTime, 
        float fHoldTime, float fEffectTime, const char[] szText, any ...)
{
...
void ShowDamageHud(int client, int channel, const char[] szText, any ...)
{
        char szBuffer[256];
        VFormat(szBuffer, sizeof(szBuffer), szText, 4);

        float fPosition[2] = {-1.0, 0.4};
        HudMsg(client, channel, fPosition, {0, 255, 255, 100}, {0, 255, 255, 100}, 0, 0.1, 0.1, 1.0, 0.0, szBuffer);
}
/***********************************************************/
/************************ SHOW DAMAGE **********************/
/***********************************************************/
void ShowDamage(char[] weapon, int attacker, int victim, int hitgroup, int count, int damage_health, int damage_armor)
{
	...
	else
	{
		...
		if(Client_IsIngame(attacker) && Client_IsIngame(victim) && attacker != victim && GetClientTeam(attacker) != GetClientTeam(victim))
		{
			...
			{
				ShowDamageHud(attacker, iChannel, "%t", "Show damage hit message body", S_hitgroup_message, damage_health, damage_armor);
			}
			else*/
			ShowDamageHud(attacker, iChannel, "%t", "Show damage hit message", damage_health);
		}
