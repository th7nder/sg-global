...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon, int &subtype, int &cmdnum, int &tickcount, int &seed, int mouse[2])
{
        if (g_bEnabled)
        {
                ...
                {
                        LogDebug(false, "OnPlayerRunCmd - Client: %i Buttons: %i", client, buttons);
                }
#endif
                ...
                {
/*
                        if ((CSGO) && ((iObserverMode[client] == -1) || iObserverMode[client] == OBS_MODE_ROAMING))
                        {
                                if ((angles[0] != fEyeAngles[client][0]) || (angles[2] != fEyeAngles[client][2]))
                                {
                                        fEyeAngles[client][0] = angles[0];
                                        fEyeAngles[client][2] = angles[2];
...
*/
                        ...
                        else if (iButtons[client] != buttons) // ( (angles[0] != fEyeAngles[client][0]) || (angles[1] != fEyeAngles[client][1]) || (angles[2] != fEyeAngles[client][2]) )
                        {
