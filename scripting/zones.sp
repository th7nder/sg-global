...
public OnPluginStart()
{
	...
	show_zones       = CreateConVar("sm_zones_show",           "1", "Whether or not show the zones on a map all the times", FCVAR_PLUGIN, true, 0.0, true, 1.0);

	// Register admin commands to control zones
	RegAdminCmd("sm_zones",     Command_SetupZones,     ADMFLAG_CUSTOM6, "Opens the zones main menu");
	RegAdminCmd("sm_actzone",   Command_ActivateZone,   ADMFLAG_CUSTOM6, "Activates a zone (by name)");
	RegAdminCmd("sm_diactzone", Command_DiactivateZone, ADMFLAG_CUSTOM6, "Diactivates a zone (by name)");
...
public OnPlayerEvents(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(StrContains(name, "death") != -1) {
		...
		for(int i = 1; i < MAXPLAYERS; i++) {
			if(IsClientInGame(i) && IsValidEdict(i) && IsPlayerAlive(i)) {
				...
				switch (real_punishment)
				{
					...
					case BOUNCE:
					{
						if (StartTouch)
						{
							...
							PrintToChat(activator,"%s Został zadany Ci DMG za próbę raszowania!", PREFIX);
							if(GetClientHealth(activator) <= 35) {
								ForcePlayerSuicide(activator);
							} else {
								SetEntityHealth(activator, (GetClientHealth(activator)-35));
							}
