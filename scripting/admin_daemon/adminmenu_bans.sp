int g_BanTime[MAXPLAYERS+1] = {0};
int g_BanTarget[MAXPLAYERS+1] = {0};
int g_BanTargetUserId[MAXPLAYERS+1] = {0};
...
public MenuHandler_BanReasonList(Menu menu, MenuAction action, int iClient, int param2) {
	if (action == MenuAction_End){
		...
		char szAuthID[64];
		if(GetClientAuthId(g_BanTarget[iClient], AuthId_Steam2, STRING(szAuthID))){
			BlockClient(iClient, g_BanTarget[iClient], szAuthID, "ban", g_BanTime[iClient], szInfo);
		}
